...
public class JDConnectSql {
	...
	public static int getMaxNum(String productId, String tableName) {
		int maxNum = 0;
		Connection con = JDConnectSql.getConn();
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT comment_totalNum FROM "
							+ tableName + " WHERE product_id='" + productId
							+ "' ORDER BY get_date DESC LIMIT 0,1;");
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				maxNum = rs.getInt("comment_totalNum");
			}
			rs.close();
			JDConnectSql.push(con);
	...
	public static int getMaxNum(String productId, String tableName,String s) {
		int maxNum = 0;
		Connection con = JDConnectSql.getConn();
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT comment_totalNum FROM "
							+ tableName + " WHERE partNumber_id='" + productId
							+ "' ORDER BY get_date DESC LIMIT 0,1;");
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				maxNum = rs.getInt("comment_totalNum");
	...
	public static Connection getConn() {
		...
		synchronized (liConn) {
			if (sqlConStr == null) {
				sqlConStr = "jdbc:mysql://" + ResolveWebPage.sqlHost + "/"
						+ sqlDbName + "?user=" + sqlName + "&password="
						+ sqlPassWord
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = JDConnectSql.bacthStmt.indexOf(stmt);
		if (s == -1)
			JDConnectSql.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = JDConnectSql.bacthStmt.indexOf(stmt);
		if (s != -1)
			JDConnectSql.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (JDConnectSql.bacthStmt.size() > 0) {
			sNum += JDConnectSql.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (JDConnectSql.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < JDConnectSql.sqlCache.size(); i++)
					stmt.addBatch(JDConnectSql.sqlCache.get(i));
			...
			JDConnectSql.sqlCache.clear();
			JDConnectSql.sqlCache = new LinkedList<String>();
		}
		JDConnectSql.sqlCache.add(sql);
